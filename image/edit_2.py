...
async def circle_with_bg(client, message):
    try:
        ...
        if not message.reply_to_message.empty:
            ...
            await message.reply_text("لماذا قمت بحذف ذالك؟؟")
        try:
        ...
        if "USER_IS_BLOCKED" in str(e):
            ...
            try:
                await message.reply_to_message.reply_text(
                    "حدث خطأ ما راسل الدعم @qad3im", quote=True
                )
            except Exception:
                return
...
async def contrast(client, message):
    try:
        ...
        if not message.reply_to_message.empty:
            ...
            contrast.enhance(1.3).save(edit_img_loc)
            await message.reply_chat_action("upload_photo")
            await message.reply_to_message.reply_photo(edit_img_loc, quote=True)
            await msg.delete()
        else:
...
def sepia(img):
    ...
    for x in range(width):
        for y in range(height):
            ...
            new_val = 0.21 * red + 0.25 * green + 0.19 * blue
            new_red = int(new_val * 1.8)
            if new_red > 255:
                new_red = 255
            new_green = int(new_val * 1.41)
            if new_green > 255:
                new_green = 255
            new_blue = int(new_val * 1.1)
            if new_blue > 255:
                new_blue = 255
